...
public class PersonServiceImpl implements PersonService {
    ...
    @Override
    public List<PersonStatisticsDTO> getSellerStatistics() {
        List<Object[]> results = personRepository.getSellerStatistics();
        return results.stream()
                .map(this::mapToPersonStatisticsDTO)
                .collect(Collectors.toList());
    }

    private PersonStatisticsDTO mapToPersonStatisticsDTO(Object[] result) {
        Long personId = ((Number) result[0]).longValue(); // seller_id
        String personName = (String) result[1]; // name
        Long revenue = ((Number) result[2]).longValue(); // revenue
        return new PersonStatisticsDTO(revenue, personName, personId);
    }
